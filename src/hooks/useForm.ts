...
const useForm = () => {
    ...
    const schemaWithDate: SchemaOf<ITaskForm> = yup.object().shape({
        deadline: yup.number()
            .test(
                'is-Valid-Date',
    ...
    const schemaWithOutDate: SchemaOf<ITaskForm> = yup.object().shape({
        deadline: yup.number(),
        status: yup.string()
            .required('Выберите корректный статус')
            .test(
    ...
    const onSubmitEdit = (task: ITask): void => {
        dispatch(editTask(task))
    }

    const onSubmitCreate = (task: ITask): void => {
